...
const createProjectKeys = (command, options) => {
  ...
  files.basePath = options.routes === 'true' ? command.paths.templ : command.paths.templ_routes;
};

// Create folder for new template files
const createProjectFolder = (command, options) => {
  if (!fs.existsSync(path.join(files.installPath, name))) {
    // Create folder
    files.createFolder(files.installPath, name);
    // Update path
    files.installPath = name;
  } else {
    console.log(chalk.red('Error: The project folder already exists!'));
    process.exit();
    // Clear
