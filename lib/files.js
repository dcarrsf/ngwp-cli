...
function processFile(folder, file, dirs) {
  if (folder) {
    ...
    // Create folder
    console.log(ipath, dirs, folder);
    // createFolder(ipath, dirs, folder);
    // Traverse its contents
    traverseFolder(troot, path.join(dirs, folder), processFile);
  } else {
    console.log(chalk.cyan('file: '), file);
    // Create file
    console.log(ipath, dirs, file);
...
module.exports = {
  ...
  install: function() {
    ...
    // Copy files
    traverseFolder(troot, templ, processFile);
    // Message
    console.log(chalk.yellow('\nThe installation was successful. Now run generators to add content...\n'));
  }
};